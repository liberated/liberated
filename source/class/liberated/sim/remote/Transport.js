 ...
 */
qx.Class.define("liberated.sim.remote.Transport",
{
  ...
  members :
  {
    ...
    send : function()
    {
      ...
      liberated.sim.Simulator.post(this.__request);
  ...
  defer : function()
  {
    ...
    qx.Class.patch(qx.io.remote.Exchange, liberated.sim.remote.MExchange);
    
    // Similarly, for qx.io.remote.Request, except it can be a simple 
    // include since it's only adding a property.
    qx.Class.include(qx.io.remote.Request, liberated.sim.remote.MRequest);

    // Patch qx.io.remote.Rpc with our own createRequest() method that 
    // supports setting the simulate property of the request.
    qx.Class.patch(qx.io.remote.Rpc, liberated.sim.remote.MRpc);
    
    // Register ourself with qx.io.remote.Exchange
    qx.io.remote.Exchange.registerType(liberated.sim.remote.Transport,
                                       "liberated.sim.remote.Transport");
    
    // Add ourself as the last tried transport
    qx.io.remote.Exchange.typesOrder.push("liberated.sim.remote.Transport");
  }
});
